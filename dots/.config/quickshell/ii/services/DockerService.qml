...
Singleton {
    ...
    Process {
        ...
        stdout: SplitParser {
            onRead: data => {
                try {
                    ...
                    if (["start","stop","die","died","kill","restart","pause","unpause",
                         "create","destroy","remove","cleanup"].includes(act)) {
        ...
        onRunningChanged: {
            if (!running) restartEventsTimer.restart()
        }
    }
    ...
    Timer {
        ...
        interval: 5000
        repeat: false
        onTriggered: { if (root.dockerAvailable) eventsProc.running = true }
    }
            ...
            try {
                ...
                const parsed = raw.map(c => {
                    ...
                    for (const [cp, hbs] of Object.entries(pb)) {
                        if (hbs) hbs.forEach(b => {
                            if (b.HostPort) ports.push({
                                containerPort: cp,
                                hostPort: b.HostPort,
    ...
    function _startMemStats() {
        if (!root.dockerAvailable || root.runningCount === 0) return
    ...
    Process {
        ...
        command: ["sh", "-c",
            "docker stats --no-stream --format '{{.ID}} {{.MemUsage}}' 2>/dev/null || true"]
        running: false
        stdout: StdioCollector { id: memStatsOut }
        onExited: {
            try {
                ...
                lines.forEach(line => {
                    ...
                    if (m) {
                        ...
                        if (u.startsWith('k'))      mb = v / 1024
                        else if (u.startsWith('m')) mb = v
                        else if (u.startsWith('g')) mb = v * 1024
                        else if (u.startsWith('t')) mb = v * 1024 * 1024
                        else                        mb = v / (1024 * 1024)
                    }
                    map[shortId] = mb
                    total += mb
                ...
                const updated = root.containers.map(c => {
                    ...
                    return Object.assign({}, c, { memMb: mb })
                })
                root._memStats = map
    ...
    function openLogs(containerId) {
        ...
        const script = [
            ...
            "  kitty --hold -- docker logs -f " + containerId,
            "elif command -v alacritty >/dev/null 2>&1; then",
            "  alacritty --hold -e docker logs -f " + containerId,
            "elif command -v foot >/dev/null 2>&1; then",
            "  foot -e sh -c 'docker logs -f " + containerId + "; read -p \"Press Enter...\"'",
            "elif command -v wezterm >/dev/null 2>&1; then",
            "  wezterm start -- sh -c 'docker logs -f " + containerId + "; read -p \"Press Enter...\"'",
            "elif command -v gnome-terminal >/dev/null 2>&1; then",
            "  gnome-terminal -- sh -c 'docker logs -f " + containerId + "; read -p \"Press Enter...\"'",
            "fi"
        ].join("\n")
    ...
    function openShell(containerId) {
        ...
        const script = [
            "if command -v kitty >/dev/null 2>&1; then",
            "  kitty -- docker exec -it " + containerId + " sh",
            "elif command -v alacritty >/dev/null 2>&1; then",
            "  alacritty -e docker exec -it " + containerId + " sh",
            "elif command -v foot >/dev/null 2>&1; then",
            "  foot -e docker exec -it " + containerId + " sh",
            "elif command -v wezterm >/dev/null 2>&1; then",
            "  wezterm start -- docker exec -it " + containerId + " sh",
            "elif command -v gnome-terminal >/dev/null 2>&1; then",
            "  gnome-terminal -- docker exec -it " + containerId + " sh",
            "fi"
        ].join("\n")
        Quickshell.execDetached(["sh", "-c", script])
    }
    ...
    function toggleDockerService(enable) {
        root.isLoading = true
        serviceToggleProc.command = [
            "/bin/sh", "-c",
            enable
                ? "pkexec /usr/bin/systemctl start docker.service docker.socket"
                : "pkexec /usr/bin/systemctl stop docker.service docker.socket"
        ]
